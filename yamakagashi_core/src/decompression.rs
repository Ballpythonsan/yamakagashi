...
pub fn image_decompression(yamakagashi_bytes: &Vec<u8>, number_of_colors: u8, size:(u32, u32)) -> Vec<u8> {
    ...
    for (select_color, compressed_page) in yamakagashi.iter().enumerate() {
        ...
        for (i, page_row) in compressed_page.iter().enumerate() {
            ...
            let mut skip = 0;
            for (unit_size, (unit_denom, unit_coeffs)) in page_row {

                let temp_unit = unit_decompression(*unit_size as usize,* unit_denom, unit_coeffs);
...
fn unit_decompression(unit_size:usize, unit_denom: i32, unit_coeffs:&Vec<i32>) -> Vec<u8> {

    assert_eq!(unit_size, unit_coeffs.len());
    let mut temp_unit: Vec<Ratio<i32>> = vec![Ratio::ZERO; unit_size];

    let mut zero_run_point = 0;
    for (i, &coeff) in unit_coeffs.iter().rev().enumerate() {if coeff != 0 {zero_run_point = i+1; break;}}

    let x:Vec<Ratio<i32>> = 
        if unit_size%2 == 0{(0..unit_size).map(|i| Ratio::new(-(unit_size as i32)+1 + 2*i as i32, 2)).collect::<Vec<Ratio<i32>>>()}
        else{(0..unit_size).map(|i| Ratio::from_integer((-(unit_size as i32)+1)/2 + i as i32)).collect::<Vec<Ratio<i32>>>()};
    let mut power_x = vec![Ratio::ONE; unit_size];
    for (_, &coeff) in (0..zero_run_point).zip(unit_coeffs) {
        temp_unit.iter_mut().zip(power_x.iter()).for_each(|(a,b)| *a += *b*coeff/unit_denom);
        power_x.hadamard_product(&x);
    }

    temp_unit.iter().map(|a| a.round().to_integer() as u8).collect()
}

fn organize(yamakagashi_bytes: &Vec<u8>, number_of_colors: u8, size: (u32, u32)) -> Vec<Vec<LinkedList<(u8, (i32, Vec<i32>))>>> {

    let mut yamakagashi: Vec<Vec<LinkedList<(u8, (i32, Vec<i32>))>>> = Vec::with_capacity(number_of_colors as usize);
    ...
    for _ in 0..number_of_colors {
        ...
        for _ in 0..size.0 {
            ...
            let mut yamakagashi_units: LinkedList<(u8, (i32, Vec<i32>))> = LinkedList::new();
            
            let mut row_size = 0;
            while row_size < size.1 {
                let unit_size = yamakagashi_bytes[index];
                index += 1;
                let unit_denom = i32::from_be_bytes(yamakagashi_bytes[index..index+4].try_into().unwrap());
                index += 4;
                let mut unit_coeffs = Vec::with_capacity(unit_size as usize);
                for _ in 0..unit_size {
                    unit_coeffs.push(i32::from_be_bytes(yamakagashi_bytes[index..index+4].try_into().unwrap()));
                    index += 4;
                }

                yamakagashi_units.push_back((unit_size,(unit_denom, unit_coeffs)));
